...
def get_forecast(city):
    ...
    if response.status_code == 200:
        ...
        for forecast in forecast_list:
            ...
            date = date_time.split(" ")[0]  # Extract date only (YYYY-MM-DD)
            time = date_time.split(" ")[1]  # Extract time (HH:MM:SS)

            # Select data for 12:00 PM (or closest available)
            if time == "12:00:00":
                daily_forecast[date] = forecast["main"]["temp"]

        dates = list(daily_forecast.keys())
        temps = list(daily_forecast.values())
...
with st.sidebar:
    ...
    if st.sidebar.button("📈 Weather Analysis"):
        st.switch_page("pages/weather_analysis.py")  # Navigates back to main.py 
